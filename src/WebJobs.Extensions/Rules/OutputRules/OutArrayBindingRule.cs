...
internal class OutArrayBindingRule<TAttribute, TMessage> : IBindingRule<TAttribute> where TAttribute : Attribute
{
    private OutputBindingRule<TAttribute> _outputRule;

    public OutArrayBindingRule(OutputBindingRule<TAttribute> outputRule)
    {
    ...
    public async Task OnFunctionExecutedAsync(TAttribute attribute, Type parameterType, object item, IDictionary<string, object> invocationState)
    {
        ...
        TMessage[] messages = (TMessage[])item;
